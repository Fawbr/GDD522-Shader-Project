...
public class MiddleBatteryAlpha : MonoBehaviour
{
    Image batteryNode;
    [SerializeField] BatteryScript batteryScript;
    float timeThermalChunk, timeEnemyChunk, timeflashlightChunk;
    float thermalNodeDrain, enemyNodeDrain, flashlightNodeDrain;
    private void Start()
    {
    ...
    void UpdateBattery(int batteryChunkType, float currentBatteryValue)
    {
        ...
        {
            if (currentBatteryValue <= batteryScript.thermalBatteryChunk * 2 && currentBatteryValue >= batteryScript.thermalBatteryChunk * 1)
            {
            ...
            else if (currentBatteryValue <= batteryScript.thermalBatteryChunk * 1)
            {
        ...
        {
            if (currentBatteryValue <= batteryScript.enemyBatteryChunk * 2 && currentBatteryValue >= batteryScript.enemyBatteryChunk * 1)
            {
            ...
            else if (currentBatteryValue <= batteryScript.enemyBatteryChunk * 1)
            {
                batteryNode.color = new Color(batteryNode.color.r, batteryNode.color.g, batteryNode.color.b, 0);
            }
            else
        ...
        {
            if (currentBatteryValue <= batteryScript.flashlightBatteryChunk * 2 && currentBatteryValue >= batteryScript.flashlightBatteryChunk * 1)
            {
            ...
            else if (currentBatteryValue <= batteryScript.flashlightBatteryChunk * 1)
            {
                batteryNode.color = new Color(batteryNode.color.r, batteryNode.color.g, batteryNode.color.b, 0);
            }
            else
